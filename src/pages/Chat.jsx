...
const Chat = () => {
        ...
        async (position) => {
          ...
          try {
            ...
            if (result.success) {
              ...
              throw new Error(result.error || "Send location failed");
            }
          } catch (error) {
      ...
      navigator.mediaDevices
        ...
        .then((stream) => {
          ...
          mediaRecorder.onstop = () => {
            const blob = new Blob(voiceRecorderRef.current.chunks, {
              type: "audio/webm",
            });
  ...
  const handleConfirmEdit = async () => {
    if (
      !editModal?.newText.trim() ||
      editModal.newText.trim() === editModal.message.content
    ) {
      setEditModal(null);
      return;
    }
  ...
  const renderMessageContent = (message) => {
    if (message.is_deleted)
      return (
        <em className="text-white/50 italic flex items-center gap-2">
      ...
      case "image":
        return (
          <img
            ...
            alt="Shared"
            className="rounded-xl w-auto max-h-60 shadow-lg cursor-pointer hover:opacity-90 transition-opacity"
            onClick={() =>
              setMediaPreview({ url: message.media_url, type: "image" })
            }
          />
        );
      case "image_once":
        if (message.sender_id === user.id) {
          return (
            <div className="flex items-center gap-2 text-indigo-300 bg-black/20 p-2 md:p-3 rounded-xl border border-indigo-500/30 text-xs md:text-sm">
              <EyeOff size={16} /> <span>1x View Photo sent</span>
            </div>
          );
        }
        return (
          <button
            ...
            className="flex items-center gap-3 text-white bg-gradient-to-r from-pink-500 to-indigo-500 p-2 md:p-3 rounded-xl shadow-lg hover:shadow-xl transition-all font-semibold text-xs md:text-sm"
          >
      ...
      case "video":
        return (
          <video
            src={message.media_url}
            controls
            className="rounded-xl w-auto max-h-60 shadow-lg border border-white/10"
          />
        );
      case "voice":
      ...
      case "location":
        return (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 text-emerald-400 font-medium hover:underline bg-emerald-500/10 p-3 rounded-xl"
          >
            <MapPin size={18} />
    ...
    <div className="flex h-[100dvh] bg-[#09090e] text-slate-100 overflow-hidden font-sans selection:bg-indigo-500/30 w-full relative">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden flex z-0">
        <motion.div
          ...
          className="absolute -top-[20%] -left-[10%] w-[70vw] h-[70vw] rounded-full bg-indigo-600/10 blur-[100px]"
        />
        <motion.div
          animate={{ rotate: -360 }}
          transition={{ repeat: Infinity, duration: 200, ease: "linear" }}
          className="absolute -bottom-[20%] -right-[10%] w-[60vw] h-[60vw] rounded-full bg-purple-600/10 blur-[120px]"
        />
      </div>
          ...
          >
            ...
            <div className="p-3 md:p-4 border-b border-indigo-500/10 bg-white/[0.01] relative overflow-hidden">
              ...
              <div className="flex justify-between items-center mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h1 className="text-lg font-black bg-clip-text text-transparent bg-gradient-to-r from-white to-indigo-300">
                      ChatSSLock
                    </h1>
                ...
                <div className="flex gap-2">
                  ...
                  {isOwner && (
                    <button
                      ...
                      className="w-9 h-9 bg-pink-500/10 hover:bg-pink-500/20 rounded-xl flex items-center justify-center transition-all border border-pink-500/20 shadow-lg"
                    >
              ...
              <div className="relative group z-50">
                <Search
                  size={16}
                  className="absolute left-3.5 top-1/2 -translate-y-1/2 text-indigo-400/50 group-focus-within:text-indigo-400 transition-colors"
                />
                <input
                  type="text"
                  placeholder="Global Search (ID or Name)..."
                  value={searchQuery}
                  onChange={(e) => handleSearchUser(e.target.value)}
                  className="w-full bg-[#050508]/50 border border-indigo-500/20 rounded-xl pl-10 pr-4 py-2.5 text-sm text-slate-200 placeholder:text-slate-600 outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all font-medium shadow-inner"
                />
              </div>
              ...
              <AnimatePresence>
                {searchQuery && (
                  ...
                  >
                    {searching ? (
                      <div className="p-4 text-center text-indigo-400 text-sm animate-pulse flex items-center justify-center gap-2">
                      ...
                      <div className="max-h-72 overflow-y-auto custom-scrollbar p-1">
                        {searchResults.map((result, i) => (
                          <motion.div
                            ...
                            className="flex items-center gap-3 p-3 cursor-pointer hover:bg-indigo-500/20 transition-all rounded-xl border border-transparent hover:border-indigo-500/30"
                          >
                            <div className="relative flex-shrink-0">
                              <img
                                src={
                                  result.avatar_url ||
                                  `https://api.dicebear.com/7.x/initials/svg?seed=${result.username}`
                                }
                                alt={result.username}
                                className="w-11 h-11 rounded-xl object-cover"
                              />
                              <div
                                className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-[#1a1a2e] ${result.is_online ? "bg-emerald-400" : "bg-slate-600"}`}
                              />
                            </div>
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2 mt-0.5">
                                <span className="text-[10px] text-indigo-300 font-mono tracking-wider bg-black/40 px-1.5 py-0.5 rounded border border-indigo-500/20">
                                  {result.unique_id}
                                </span>
                                <span
                                  className={`text-[9px] font-bold uppercase ${result.is_online ? "text-emerald-400" : "text-slate-500"}`}
                                >
                      ...
                      <div className="p-5 text-center text-rose-400 text-xs font-bold uppercase tracking-widest bg-rose-500/5">
                        <Search size={24} className="mx-auto mb-2 opacity-50" />{" "}
                        No Match Found on Global Network
                      </div>
                    )}
                  </motion.div>
                )}
            ...
            <div className="flex-1 overflow-y-auto p-2 md:p-3 space-y-1 custom-scrollbar z-10 relative">
              {conversations.length === 0 && !searchQuery ? (
                <div className="h-[80%] flex flex-col items-center justify-center text-center opacity-60 px-4">
                  <MessageCircle
                    size={40}
                    className="mb-3 text-indigo-500/40"
                  />
                  <p className="text-sm font-semibold text-slate-400">
                    No active links
                  </p>
                  <p className="text-[10px] text-slate-500 mt-2 uppercase tracking-widest text-center max-w-[200px]">
                    Use Global Search to find secure nodes
                  </p>
                </div>
              ) : null}
              ...
              {conversations.map((conv, i) => (
                <motion.div
                  ...
                  className={`group flex items-center gap-3 p-2.5 rounded-[20px] cursor-pointer transition-all duration-300 border ${currentConversation?.id === conv.id ? "bg-gradient-to-r from-indigo-500/10 to-transparent border-l-2 border-l-indigo-500 border-y-transparent border-r-transparent" : "border-transparent hover:bg-white/[0.02]"}`}
                >
                  <div className="relative">
                    <img
                      src={
                        conv.otherUser?.avatar_url ||
                        `https://api.dicebear.com/7.x/initials/svg?seed=${conv.otherUser?.username}`
                      }
                      alt={conv.otherUser?.username}
                      className="w-12 h-12 rounded-[18px] object-cover shadow-inner"
                    />
                    <div
                      className={`absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-[#12121e] md:border-[#0a0a0f] transition-colors ${conv.otherUser?.is_online ? "bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,0.6)]" : "bg-slate-500"}`}
                    />
                  </div>
                  <div className="flex-1 min-w-0 pr-1">
                    <div className="flex justify-between items-center mb-0.5">
                      <span
                        className={`font-bold text-[13px] md:text-sm truncate ${currentConversation?.id === conv.id ? "text-indigo-400" : "text-slate-200 group-hover:text-white"}`}
                      >
                        {conv.otherUser?.username}
                      </span>
                      <span className="text-[9px] uppercase font-bold text-slate-500 tracking-widest whitespace-nowrap ml-2">
                        {formatConversationTime(conv.lastMessage?.created_at)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center mt-1">
                      <span className="text-[12px] text-slate-400 truncate pr-2 font-medium">
                      ...
                      {conv.unreadCount > 0 && (
                        <span className="min-w-[18px] h-[18px] bg-gradient-to-br from-indigo-500 to-purple-500 rounded-full text-[9px] font-black text-white flex items-center justify-center px-1 shadow-[0_0_10px_rgba(99,102,241,0.5)]">
                          {conv.unreadCount}
                        </span>
                      )}
                    </div>
            ...
            <div className="px-3 pb-3 pt-2 bg-gradient-to-t from-[#050508] to-transparent z-20 space-y-2">
              <div className="flex items-center justify-between px-1">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[9px] text-emerald-400/80 font-bold uppercase tracking-widest">
                    {conversations.filter((c) => c.otherUser?.is_online).length}{" "}
                    online
                  </span>
                </div>
                <span className="text-[9px] text-slate-600 font-mono">
                  {conversations.length} link
              ...
              <div className="bg-[#12121e] border border-white/5 p-2 rounded-[20px] flex items-center justify-between shadow-xl hover:border-indigo-500/20 transition-colors">
                <div
                  onClick={() => navigate(`/user/${user?.unique_id}`)}
                  className="flex items-center gap-3 min-w-0 pr-2 pl-1 cursor-pointer group hover:bg-white/5 p-1 -m-1 rounded-[16px] transition-colors"
                ...
                >
                  <div className="relative">
                    <img
                      ...
                      className="w-10 h-10 rounded-[14px] border border-indigo-500/20 object-cover flex-shrink-0"
                    />
                  ...
                  <div className="flex-1 min-w-0 overflow-hidden">
                    <div className="text-[13px] font-bold truncate text-white leading-tight">
                      {user?.username}
                    </div>
                ...
                <button
                  onClick={logout}
                  className="w-10 h-10 flex-shrink-0 bg-red-500/5 hover:bg-red-500/20 rounded-xl text-red-500 flex items-center justify-center transition-colors"
                  title="Disconnect"
                >
      ...
      {/* Main Container Area */}
      <div
        className={`flex-1 flex flex-col relative transition-all duration-300 z-10 h-[100dvh] overflow-hidden`}
      >
        ...
        {currentConversation ? (
          <div className="flex flex-col h-full w-full bg-[#0a0a12] relative z-10 overflow-hidden">
            ...
            <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
              <motion.div
                animate={{ x: [0, 50, 0, -50, 0], y: [0, -40, 20, -20, 0] }}
                transition={{
                  repeat: Infinity,
                  duration: 20,
                  ease: "easeInOut",
                }}
                className="absolute top-[5%] left-[5%] w-[35vw] h-[35vw] rounded-full bg-indigo-600/12 blur-[90px]"
              />
              <motion.div
                animate={{ x: [0, -30, 25, -15, 0], y: [0, 30, -25, 15, 0] }}
                transition={{
                  repeat: Infinity,
                  duration: 25,
                  ease: "easeInOut",
                }}
                className="absolute bottom-[10%] right-[5%] w-[30vw] h-[30vw] rounded-full bg-pink-600/10 blur-[80px]"
              />
              <motion.div
                ...
                className="absolute top-[45%] left-[40%] w-[20vw] h-[20vw] rounded-full bg-purple-500/6 blur-[70px]"
              />
              <motion.div
                ...
                className="absolute top-[10%] right-[10%] w-[40vw] h-[40vw] rounded-full bg-blue-500/10 blur-[100px]"
              />
              <motion.div
                ...
                className="absolute bottom-[20%] left-[10%] w-[35vw] h-[35vw] rounded-full bg-rose-500/8 blur-[90px]"
              />
              <motion.div
                ...
                className="absolute top-[30%] left-[15%] w-1.5 h-1.5 rounded-full bg-indigo-400 shadow-[0_0_15px_rgba(99,102,241,1)] opacity-60"
              />
              <motion.div
                animate={{ scale: [1, 2, 1] }}
                transition={{
                ...
                className="absolute top-[65%] right-[20%] w-1 h-1 rounded-full bg-pink-400 shadow-[0_0_12px_rgba(236,72,153,1)] opacity-50"
              />
              <motion.div
                ...
                className="absolute top-[20%] right-[28%] w-1 h-1 rounded-full bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,1)] opacity-40"
              />
              <div
                className="absolute inset-0 opacity-15"
                style={{
                  backgroundImage:
                    "repeating-linear-gradient(0deg,transparent,transparent 39px,rgba(99,102,241,0.04) 40px),repeating-linear-gradient(90deg,transparent,transparent 39px,rgba(99,102,241,0.04) 40px)",
                }}
              />
            </div>
            ...
            <div className="px-3 md:px-5 pt-16 pb-4 md:py-3 bg-[#0a0a10]/95 backdrop-blur-xl border-b border-indigo-500/10 flex items-center justify-between gap-2 z-20 shadow-md sticky top-0">
              <div className="flex items-center gap-2 mt-3 md:mt-0">
                {" "}
                {/* mt-2 untuk tambahan jarak internal */}
                {/* Tombol Back */}
                <button
                  onClick={() => {
                ...
                <div
                  ...
                  onClick={() =>
                    navigate(
                      `/user/${currentConversation.otherUser?.unique_id}`,
                    )
                  }
                >
                  <div className="relative flex-shrink-0">
                    <img
                      src={
                        ...
                        `https://api.dicebear.com/7.x/initials/svg?seed=${currentConversation.otherUser?.username}`
                      }
                    ...
                    />
                    <div
                      className={`absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-[#0a0a12] ${currentConversation.otherUser?.is_online ? "bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.8)] animate-pulse" : "bg-slate-500"}`}
                    />
                  </div>
                  <div className="min-w-0">
                    <div className="font-extrabold text-slate-100 text-[15px] md:text-[14px] truncate max-w-[140px] leading-tight group-hover:text-indigo-300 transition-colors">
                      {currentConversation.otherUser?.username}
                    </div>
                    <div
                      className={`flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest mt-1 ${currentConversation.otherUser?.is_online ? "text-emerald-400" : "text-slate-500"}`}
                    >
                      {currentConversation.otherUser?.is_online ? (
                        <>
              ...
              <div className="flex items-center gap-1.5 mt-2 md:mt-0">
                <button
                  ...
                  className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors shadow-sm ${showMsgSearch ? "bg-indigo-500/20 text-indigo-300 border border-indigo-500/30" : "bg-indigo-500/5 hover:bg-indigo-500/20 border border-indigo-500/10 text-indigo-400"}`}
                  title="Cari pesan"
                >
                  <Search size={16} />
                </button>
                ...
                <button
                  ...
                  className="w-10 h-10 bg-indigo-500/5 hover:bg-indigo-500/20 border border-indigo-500/10 rounded-xl flex items-center justify-center text-indigo-400 transition-colors"
                  title="Info User"
                >
            ...
            <AnimatePresence>
              {showMsgSearch && (
                <motion.div
                  ...
                  className="px-3 md:px-5 pt-3 overflow-hidden z-10 w-full max-w-lg mx-auto"
                >
                  <div className="relative flex items-center shadow-lg">
                    <Search
                      size={14}
                      className="absolute left-3 text-indigo-400/70"
                    />
                    <input
                      autoFocus
                      type="text"
                      ...
                      className="w-full bg-[#12121e]/90 border border-indigo-500/20 rounded-xl pl-9 pr-8 py-2 text-sm text-slate-200 placeholder:text-slate-500 outline-none focus:ring-1 focus:ring-indigo-500/50 backdrop-blur-md"
                    />
                    {msgSearchQuery && (
                      <button
            ...
            <div
              ...
              className="flex-1 overflow-y-auto px-3 md:px-8 py-4 flex flex-col gap-3 relative z-10 custom-scrollbar scroll-smooth"
            >
              {/* E2E Encryption Banner - Full */}
              <motion.div
                initial={{ opacity: 0, y: -10 }}
                animate={{ opacity: 1, y: 0 }}
                className="mx-auto w-full max-w-lg mb-5 mt-3"
              >
                <div className="bg-amber-500/8 border border-amber-500/25 rounded-2xl px-4 py-3 shadow-[0_4px_20px_rgba(245,158,11,0.07)] backdrop-blur-xl">
                  ...
                  <p className="text-[9px] md:text-[10px] text-amber-400/70 font-medium leading-relaxed tracking-wide text-center">
                    🔒 Seluruh pesan, foto, video &amp; suara dienkripsi{" "}
                    <strong>End-to-End</strong> oleh sistem keamanan{" "}
                    <strong>RAIHAN_OFFICIAL0307 × VISUALCODEPO</strong>. Tidak
                    ada pihak ketiga — termasuk server kami — yang dapat membaca
                    atau menyadap percakapan ini. Pesan Anda aman dari
                    pengintaian, peretasan, dan kebocoran data. Privasi 100%
                    terjaga.
                  </p>
                </div>
              </motion.div>
                ...
                messages
                  ...
                  .map((message, i) => (
                    <motion.div
                      ...
                      className={`flex gap-2 w-fit max-w-[85%] md:max-w-[70%] group ${message.sender_id === user.id ? "self-end flex-row-reverse" : "self-start"} mb-8 pl-1 pr-1`}
                    >
                      {message.sender_id !== user.id && (
                        <img
                          src={
                            message.sender?.avatar_url ||
                            `https://api.dicebear.com/7.x/initials/svg?seed=${message.sender?.username}`
                          }
                          alt={message.sender?.username}
                          className="w-7 h-7 md:w-8 md:h-8 rounded-[10px] object-cover self-end mb-1 shadow-md flex-shrink-0 ring-1 ring-white/10"
                        />
                      )}

                      <div
                        className={`p-3 md:p-3.5 rounded-[20px] relative backdrop-blur-md shadow-lg transition-transform flex flex-col ${message.sender_id === user.id ? "bg-indigo-600/90 rounded-br-[6px] text-white border border-indigo-500/50" : "bg-[#1a1a24]/90 border border-white/5 rounded-bl-[6px] text-slate-200"}`}
                      >
                        {message.reply_to && (
                          <div
                            className={`mb-2 p-2 rounded-xl border-l-4 text-[11px] cursor-pointer group/reply max-w-full overflow-hidden ${
                              message.sender_id === user.id
                          ...
                          >
                            <div className="font-bold text-indigo-400 text-[9px] uppercase tracking-tighter mb-1">
                              Replying to message
                            </div>
                            <div className="break-words break-all opacity-80 leading-snug">
                        ...
                        {/* Quick actions popup - Restyled to rest at bottom corner */}
                        <div
                          className={`
  absolute flex items-center gap-1 transition-all duration-300 z-[40]
  -bottom-9 p-1 rounded-xl shadow-xl border border-white/10 backdrop-blur-xl
  ${
    message.sender_id === user.id
      ? "right-2 bg-indigo-900/90" // Digeser ke kiri (right-2) agar tidak overlap dengan waktu tapi ada dibawah bubble
      : "left-2 bg-[#12121e]/95"
  }
  ${isMobile ? "opacity-100 visible" : "opacity-0 invisible translate-y-2 group-hover:translate-y-0 group-hover:opacity-100 group-hover:visible"}
`}
                        >
                          {/* Copy Button */}
                          <button
                            ...
                            className="w-7 h-7 rounded-lg hover:bg-white/10 flex items-center justify-center text-slate-300 active:scale-90 transition-all border border-transparent"
                            title="Copy"
                          >
                            {copiedId === message.id ? (
                          ...
                          {canEdit(message) && (
                            <button
                              ...
                              className="w-7 h-7 rounded-lg hover:bg-indigo-500/30 flex items-center justify-center text-indigo-300 active:scale-90 transition-all"
                              title="Edit"
                            >
                              <Edit3 size={12} />
                            </button>
                          ...
                          {message.sender_id === user.id &&
                            !message.is_deleted && (
                              <button
                                ...
                                className="w-7 h-7 rounded-lg hover:bg-red-500/30 flex items-center justify-center text-red-400 active:scale-90 transition-all"
                                title="Hapus"
                              >
                                <Trash2 size={12} />
                              </button>
                        ...
                        <div className="text-[13px] md:text-[14px] leading-relaxed break-words break-all font-medium">
                          {renderMessageContent(message)}
                        </div>

                        <div
                          className={`flex items-center justify-end gap-1 mt-1 text-[9px] md:text-[10px] font-bold tracking-widest ${message.sender_id === user.id ? "text-indigo-200/70" : "text-slate-500"}`}
                        >
              ...
              <AnimatePresence>
                {showScrollBottom && (
                  <motion.button
                    ...
                    onClick={scrollToTop}
                    className="sticky bottom-4 mx-auto w-10 h-10 bg-indigo-500 hover:bg-indigo-400 text-white rounded-full flex items-center justify-center shadow-[0_4px_20px_rgba(99,102,241,0.5)] z-[100] transition-colors"
            ...
            <div className="px-3 py-1.5 bg-transparent z-20 flex gap-2 overflow-x-auto custom-scrollbar opacity-70 hover:opacity-100 transition-opacity">
              ...
              {replyingTo && (
                <div className="flex items-center gap-2 px-3 py-1 bg-indigo-500/20 rounded-full text-[10px] font-bold uppercase tracking-widest text-indigo-300 border border-indigo-500/30 flex-shrink-0">
                  <Reply size={10} /> Replying:{" "}
                  <span className="truncate max-w-[100px] lowercase font-mono">
                    {replyingTo.content.substring(0, 15)}
                  </span>
                  <button
            ...
            <AnimatePresence>
              {mediaPreview && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: 10 }}
                  className="absolute bottom-20 left-3 md:left-6 z-[60]"
                >
                  <div className="bg-[#12121e]/95 backdrop-blur-2xl border border-white/10 rounded-[20px] p-2 shadow-2xl relative">
                    <button
                      onClick={() => setMediaPreview(null)}
                      className="absolute -top-2 -right-2 w-6 h-6 bg-rose-500 hover:bg-rose-600 rounded-full flex items-center justify-center text-white shadow-lg transition-colors z-10"
                    >
                      <X size={12} />
                    </button>
                    {mediaPreview.type === "image" && (
                      <img
                        ...
                        alt="Preview"
                        className="h-24 md:h-32 rounded-[14px] object-cover"
                      />
                    )}
                    {mediaPreview.type === "video" && (
                      <video
                        src={mediaPreview.url}
                        controls
                        className="h-24 md:h-32 rounded-[14px]"
                      />
                    )}
            ...
            {sendError && (
              <div className="absolute top-20 left-1/2 -translate-x-1/2 bg-red-500 text-white text-[10px] font-bold uppercase tracking-widest px-4 py-1.5 rounded-full shadow-[0_0_20px_rgba(239,68,68,0.5)] z-50 whitespace-nowrap animate-bounce">
                {sendError}
              </div>
            )}

            {/* Input Form Wrapper - FIXED FOR MOBILE BOTTOM */}
            <div className="px-2 md:px-4 py-2 md:py-1 bg-[#0a0a10]/95 backdrop-blur-xl border-t border-white/5 relative z-20 w-full safe-bottom">
              <div className="max-w-5xl mx-auto flex items-end gap-1 md:gap-2 bg-[#14141e] border border-white/5 rounded-[20px] md:rounded-[24px] p-3 shadow-2xl focus-within:ring-1 focus-within:ring-indigo-500/50 transition-all">
                <div className="flex gap-0.5 pb-1 pl-1">
                  <button
                    onClick={() => setShowEmojiPicker(!showEmojiPicker)}
                    className="w-9 h-9 flex items-center justify-center text-indigo-400 hover:bg-white/5 rounded-full transition-colors flex-shrink-0"
                  >
                    <Smile size={20} />
                  </button>
                  <button
                    onClick={() => fileInputRef.current?.click()}
                    className="w-9 h-9 flex items-center justify-center text-slate-400 hover:bg-white/5 rounded-full transition-colors flex-shrink-0"
                  >
                    <Image size={20} />
                  </button>
                </div>
                ...
                <div className="flex items-center gap-1 pb-1 pr-1">
                  ...
                  <button
                    ...
                    className={`w-9 h-9 flex items-center justify-center rounded-full transition-all border flex-shrink-0 ${isRecording ? "bg-red-500 text-white border-red-500 animate-pulse" : "text-slate-400 border-transparent hover:text-red-400"}`}
                  >
                    <Mic size={18} />
                  </button>
                  <button
                      ...
                      <div className="flex items-start gap-4 relative z-10">
                        ...
                        <div>
                          ...
                          <p className="text-slate-400 text-[11px] leading-relaxed font-medium">
                            Pesan 1x lihat telah diaktifkan. Anda dapat mengirim
                            foto yang akan{" "}
                            <span className="text-rose-400 font-bold">
                              langsung terhapus
                            </span>{" "}
                            dari node jaringan setelah dilihat oleh penerima.
                          </p>
              ...
              <AnimatePresence>
                {showEmojiPicker && (
                  <motion.div
                    ...
                    className="absolute bottom-16 left-2 md:left-4 z-[70] shadow-[0_0_50px_rgba(0,0,0,0.8)] border border-white/10 rounded-2xl overflow-hidden"
                  >
                    <EmojiPicker
          ...
          <div className="flex-1 flex flex-col items-center justify-start md:justify-center p-4 md:p-6 relative z-10 bg-[#050508] w-full min-h-[100dvh] overflow-y-auto custom-scrollbar pt-28 pb-10 md:pt-0">
            ...
            <div className="fixed inset-0 pointer-events-none z-0">
              {/* Animated Floating Gradients (Better visibility on mobile) */}
              <motion.div
                animate={{ scale: [1, 1.2, 1], rotate: [0, 90, 0] }}
                transition={{ repeat: Infinity, duration: 15, ease: "linear" }}
                className="absolute top-10 left-[-10%] w-[60vw] max-w-[500px] aspect-square bg-indigo-500/20 blur-[100px] rounded-full"
              />
              <motion.div
              ...
              <motion.div
                ...
                className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[50vw] max-w-[400px] aspect-square bg-emerald-500/5 blur-[100px] rounded-full"
              />
            ...
            <div
              className="absolute bottom-40 right-20 w-3 h-3 bg-pink-500 rounded-full shadow-[0_0_15px_pink] opacity-40 animate-ping"
            ...
            <motion.div
              ...
              className="w-full max-w-[1000px] flex flex-col md:flex-row items-stretch justify-center gap-6 lg:gap-10 relative z-10 my-auto"
            >
              {/* Left Column: Intro Card + QR Scanner */}
        ...
        {viewOnceImage && (
          ...
          >
            <button
              ...
              className="absolute top-6 right-6 md:top-8 md:right-8 w-10 h-10 md:w-12 md:h-12 bg-white/10 hover:bg-rose-500/50 rounded-full flex items-center justify-center text-white transition-all hover:rotate-90 z-[110]"
            >
              <X size={20} />
            </button>
            <div className="p-4 text-center w-full max-w-5xl h-[100dvh] flex flex-col justify-center items-center">
              <p className="text-rose-400 font-bold mb-4 md:mb-6 flex items-center justify-center gap-2 uppercase tracking-widest text-[10px] md:text-xs shadow-black drop-shadow-xl bg-black/50 py-2 px-6 rounded-full border border-rose-500/20">
                <EyeOff size={16} /> View Once - Secure Image Link
              </p>
              <img
                src={viewOnceImage.url}
                alt="View Once"
                className="w-auto h-auto max-w-full max-h-[75vh] object-contain rounded-2xl shadow-[0_0_100px_rgba(0,0,0,0.8)] border border-white/10 select-none touch-none"
            ...
            >
              ...
              <div className="p-6">
                ...
                <div className="bg-black/30 rounded-xl p-3 mb-3 border border-white/5">
                  <p className="text-[9px] text-indigo-400 uppercase tracking-widest mb-1.5 font-bold">
                    📝 Pesan Lama
                  </p>
                  <p className="text-[13px] text-slate-400 font-medium break-words leading-relaxed">
                    {editModal.message.content}
                  </p>
                </div>
                <div className="mb-5">
                  <p className="text-[9px] text-emerald-400 uppercase tracking-widest mb-1.5 font-bold">
                    ✨ Pesan Baru
                  </p>
                  <textarea
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => setEditModal(null)}
                    className="flex-1 py-3 bg-white/5 hover:bg-white/10 rounded-xl text-sm font-bold text-slate-400 transition-colors border border-white/5"
                  >
                    Batal
                  </button>
                  <button
                    onClick={handleConfirmEdit}
                    className="flex-1 py-3 bg-indigo-500 hover:bg-indigo-400 rounded-xl text-sm font-bold text-white transition-colors shadow-lg shadow-indigo-500/30 active:scale-95"
                  >
                    Simpan ✓
                  </button>
                </div>
            ...
            >
              ...
              <div className="p-6 text-center">
                <motion.div
                  ...
                  transition={{ repeat: Infinity, duration: 2 }}
                  className="w-16 h-16 bg-red-500/10 border border-red-500/20 rounded-2xl flex items-center justify-center text-red-400 mx-auto mb-5"
                >
                  <Trash2 size={28} />
                </motion.div>
                <h3 className="font-bold text-white text-[17px] mb-2">
                  Hapus Pesan?
                </h3>
                <p className="text-[12px] text-slate-400 mb-3 leading-relaxed">
                  Pesan ini akan dihapus permanen untuk semua orang dalam
                  percakapan ini.
                </p>
                {deleteModal.content && !deleteModal.is_deleted && (
                  <div className="bg-black/30 rounded-xl p-2 mb-5 border border-white/5 text-left">
                    <p className="text-[11px] text-slate-500 italic truncate">
                      "{deleteModal.content.substring(0, 70)}
                      {deleteModal.content.length > 70 ? "…" : ""}"
                    </p>
                  </div>
                )}
                <div className="flex gap-3 mt-4">
                  <button
                    onClick={() => setDeleteModal(null)}
                    className="flex-1 py-3 bg-white/5 hover:bg-white/10 border border-white/5 rounded-xl text-sm font-bold text-slate-400 transition-colors"
                  >
                    Batal
                  </button>
                  <button
                    onClick={handleConfirmDelete}
                    className="flex-1 py-3 bg-red-500 hover:bg-red-600 active:scale-95 rounded-xl text-sm font-bold text-white transition-all shadow-lg shadow-red-500/20"
                  >
                    Hapus ✓
                  </button>
                </div>
      ...
      {/* 🛡️ PASSWORD CHANGED SECURITY MODAL */}
      <AnimatePresence>
        {passwordChanged && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            <motion.div
              ...
              className="w-full max-w-md bg-[#0a0a0f] rounded-[32px] border border-rose-500/30 shadow-[0_0_100px_rgba(244,63,94,0.15)] overflow-hidden relative"
            >
              ...
              <div className="p-8 text-center relative z-10">
                <motion.div
                  ...
                  transition={{ repeat: Infinity, duration: 2 }}
                  className="w-24 h-24 mx-auto bg-rose-500/10 border border-rose-500/20 rounded-full flex items-center justify-center shadow-[0_0_30px_rgba(244,63,94,0.2)] mb-6"
                >
                ...
                <h3 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-rose-400 to-orange-400 mb-6">
                  Password Telah Diubah
                </h3>

                <p className="text-[13px] text-slate-300 font-medium leading-relaxed mb-6 bg-white/5 p-4 rounded-2xl border border-white/5 text-left">
                  Sistem keamanan kami mendeteksi bahwa password akun Anda baru
                  saja diubah oleh Owner.
                  <br />
                  <br />
                  <span className="text-rose-300 font-bold">
                    Semua sesi aktif telah diakhiri secara otomatis demi
                    keamanan.
                  </span>{" "}
                  Silakan hubungi <strong>Owner (RAIHAN_OFFICIAL0307)</strong>{" "}
                  untuk meminta password Anda yang baru.
                </p>

                <button
                  onClick={handlePasswordChangedAck}
                  className="w-full py-4 bg-gradient-to-r from-rose-600 to-orange-600 hover:from-rose-500 hover:to-orange-500 active:scale-95 rounded-2xl text-sm font-black text-white uppercase tracking-widest transition-all shadow-[0_10px_30px_rgba(244,63,94,0.3)] flex items-center justify-center gap-2"
                >
                  <LogOut size={18} /> Mengerti & Keluar
                </button>
              </div>
            </motion.div>
          </motion.div>
...
const VoiceMessage = ({ mediaUrl }) => {
  ...
  return (
    <div className="flex items-center gap-2 md:gap-3 p-1 bg-black/40 rounded-full border border-white/5 min-w-[150px] md:min-w-[180px]">
      <button
        onClick={togglePlay}
        className="w-8 h-8 md:w-9 md:h-9 flex-shrink-0 bg-indigo-500 hover:bg-indigo-400 rounded-full flex items-center justify-center shadow-[0_0_10px_rgba(99,102,241,0.4)] text-white transition-transform active:scale-95"
      >
        {isPlaying ? (
          <Pause size={12} />
        ) : (
      ...
      <div className="flex items-center gap-1 h-4 md:h-5 flex-1 px-1 md:px-2">
        {[...Array(20)].map((_, i) => (
          <div
            key={i}
            className="flex-1 bg-indigo-300/50 rounded-full transition-all duration-150"
            style={{
